...
module fc_layer #(
    ...
    // 需要加上 neuron_idx × NUM_INPUTS 的 offset 才能定址到正確的權重
    reg [$clog2(NUM_OUTPUTS)-1:0] neuron_idx;
    assign weight_read_addr = neuron_idx * NUM_INPUTS + mac_weight_addr;

    // input_addr 直接透傳給外部
    assign input_addr = mac_input_addr;

    // ─── LIF 運算（組合邏輯，不用獨立模組）───
    // 因為 LIF 是時分多工的，我們在 FSM 裡直接算
    // 這比實例化 lif.v 更簡單（不用額外的 en/cur_in 控制）
    wire signed [MEM_WIDTH+6-1:0] decay_product;
    wire signed [MEM_WIDTH-1:0]   decayed_mem;
    wire signed [MEM_WIDTH-1:0]   new_mem;
    wire                          spike;

    assign decay_product = BETA * mem_state[neuron_idx];
    assign decayed_mem   = decay_product >>> FRAC_BITS;
    assign new_mem       = decayed_mem + mac_result;
    assign spike         = (new_mem >= THRESHOLD);

    // ─── FSM ───
    localparam S_IDLE       = 2'd0;
    localparam S_MAC_RUN    = 2'd1;
    localparam S_LIF_UPDATE = 2'd2;
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_IDLE: begin
                    if (start) begin
                        neuron_idx <= 0;
                        spk_out    <= 0;   // 清空本 time step 的 spike
                        mac_start  <= 1;   // 啟動第一個神經元的 MAC
                        state      <= S_MAC_RUN;
                    end
                end
                ...
                S_LIF_UPDATE: begin
                    ...
                    if (spike) begin
                        ...
                        // spk_out[neuron_idx] 保持 0（在 S_IDLE 已清空）
                    end
